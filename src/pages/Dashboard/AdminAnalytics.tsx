              ...
              <tbody className="divide-y divide-[var(--border)]">
                {tableData.data.map((row, i) => (
                  <tr key={row.ad_id || i} className="hover:bg-[var(--glass2)] transition-colors">
                        ...
                        <div>
                          ...
                          {row.old_price != null && (
                            <span className="block text-[9px] line-through opacity-30">
                              {(row.old_price ?? 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                            </span>
                          )}
                        </div>
                      ) : <span className="opacity-20">—</span>}
                    ...
                    <td className="px-4 py-3 font-mono text-center opacity-60">{(row.views_count ?? 0).toLocaleString('pt-BR')}</td>
                    <td className="px-4 py-3 font-mono text-center font-black">{(row.clicks_count ?? 0).toLocaleString('pt-BR')}</td>
        ...
        {tableData && tableData.pages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t border-[var(--border)]">
            <p className="text-[10px] opacity-40 font-black uppercase tracking-widest">
              Página {tableData.page} de {tableData.pages} · {(tableData.total ?? 0).toLocaleString('pt-BR')} registros
            </p>
              ...
              {Array.from({ length: Math.min(5, tableData.pages || 0) }, (_, i) => {
                ...
                return (
                  <button key={p} onClick={() => setPage(p)}
                    className={`w-8 h-8 rounded-lg text-[11px] font-black border transition-all ${p === page ? 'bg-orange text-white border-orange shadow-lg shadow-orange/20' : 'bg-[var(--glass2)] border-[var(--border)] hover:border-orange/30'
                      }`}>
                    {p}
                  </button>
                );
